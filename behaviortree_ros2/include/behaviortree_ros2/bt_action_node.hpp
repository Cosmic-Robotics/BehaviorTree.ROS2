 ...
 *
 * class FibonacciNode: public
 * RosActionNode<action_tutorials_interfaces::action::Fibonacci>
 *
 * RosActionNode will try to be non-blocking for the entire duration of the
 * call. The derived class must reimplement the virtual methods as described
 * below.
 *
...
class RosActionNode : public BT::ActionNodeBase {
 public:
   ...
   * It generally returns RUNNING, but the user can also use this callback to
   * cancel the current action and return SUCCESS or FAILURE.
   */
  ...
  /// Method used to send a request to the Action server to cancel the current
  /// goal
  void cancelGoal();
...
RosActionNode<T>::ActionClientInstance::ActionClientInstance(
  ...
  callback_executor.add_callback_group(callback_group,
                                       node->get_node_base_interface());
  action_client =
      rclcpp_action::create_client<T>(node, action_name, callback_group);
}
...
inline bool RosActionNode<T>::createClient(const std::string& action_name) {
  ...
  action_client_key_ =
      std::string(node->get_fully_qualified_name()) + "/" + action_name;

  auto& registry = getRegistry();
  auto it = registry.find(action_client_key_);
  if (it == registry.end() || it->second.expired()) {
    client_instance_ =
        std::make_shared<ActionClientInstance>(node, action_name);
  ...
  bool found = client_instance_->action_client->wait_for_action_server(
...
inline NodeStatus RosActionNode<T>::tick() {
  ...
  if (status() == NodeStatus::RUNNING) {
    ...
    if (!goal_received_) {
      ...
      auto timeout = rclcpp::Duration::from_seconds(
          double(server_timeout_.count()) / 1000);
...
inline void RosActionNode<T>::cancelGoal() {
  ...
  if (!goal_handle_) {
    if (future_goal_handle_.valid()) {
      // Here the discussion is if we should block or put a timer for the
      // waiting
      auto ret = executor.spin_until_future_complete(future_goal_handle_,
                                                     server_timeout_);
      if (ret != rclcpp::FutureReturnCode::SUCCESS) {
        // In that case the goal was not accepted or timed out so probably we
        // should do nothing.
        return;
      } else {
        goal_handle_ = future_goal_handle_.get();
